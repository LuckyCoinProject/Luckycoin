...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Luckycoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Luckycoin
    // Mac: ~/Library/Application Support/Luckycoin
    // Unix: ~/.luckycoin
#ifdef WIN32
    // Windows
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 20 * 60) // Luckycoin: changed maximum adjust to 20 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
